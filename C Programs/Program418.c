...
void EnQue(PPNODE First,int No)
{
    PNODE newn=NULL;
    PNODE temp=NULL;
...
int DeQueue(PPNODE First)
{
    ...
    if(*First == NULL)
    {
        printf("Unable to delete as queue is empty.");
        return -1;
    }
...
int main()
{
    ...
    iRet=Count(Head);
    printf("Number of elements of queue are: %d\n",iRet);
...
int main()
{
    ...
    while(iChoice != 5)
    {
        ...
        {
            case 1:
                printf("Enter number which you want to insert in queue: \n");
                scanf("%d",&iNo);
                EnQue(&Head,iNo);
            break;
